...
struct ms3d_keyframe_position_t {
	float time;
	float key[3];
};

struct ms3d_keyframe_rotation_t {
	float time;
	float key[4];
};
...
struct ms3d_joint_t {
	...
	std::vector<ms3d_keyframe_rotation_t> rotationKeys;
	std::vector<ms3d_keyframe_position_t> positionKeys;
...
private:
	MS3DModel(const MS3DModel& rhs);
	MS3DModel& operator=(const MS3DModel& rhs);
};
